...
In fact, I had to do a bridging step of converting the image to an embroidery file that the machine can actually understand using a trial version of the <a href="https://www.wilcom.com/Hatch/Products/HotcakeStoreAdmin/ProductViewer/tabid/3605/slug/Embroidery-Digitizer/Default.aspx">Hatch embroidery digitizer software</a>. And now you are ready to embroid on my textile of choice.

<img src="/Media/Photo%206.2.jpg" width="400" height="300">

<i>Photo: touch sensors pattern embroidered using conductive thread</i>

<h3>3. Test hardware and software </h3>
...
<ul>3.4. Prepare 1 row of 6 female pin headers and 5 medium to high value resistors (100 kilohm - 50 megohm). Solder <b>1 end of all 5 resistors to the <a name="firstpin">first pin</a></b> and the other end of each resistor to each of the remaining pin. An explanation of the capacitive sensors is avalaiable on the <a href="http://playground.arduino.cc/Main/CapacitiveSensor?from=Main.CapSense">Arduino website</a>.</ul>


<img src="/Media/Photo%205.4c.jpg" width="400" height="300">

<i>Photo: Row of pin headers with resistors, <a name="firstpin">first pin </a> is the upper most pin</i>
...
<ul>3.6. Finally, solder the other end of each sensor's wire to each of the female pin headers from the row above, <b> except for the [first pin](#firstpin) </b>.</ul>

<ul>3.7. Now wire up the loudspeaker, Neopixel, sensors and resistors-pin headers to the Arduino Pro Micro as in the photo below:</ul>

<img src="/Media/Photo%205.5.jpg" width="400" height="300">

A table of connections:

Component's pin ------------ Arduino's pin
...
Pin headers 1-6 ------------ 2-7, which also means 

Sensors 1-5 ---------------- 3-7

The loudspeaker PWM pin can be redefined as any other PWM ~ pin in the [final sketch](/Arduino%20Sketch/Musical_wristband_rev0.2.ino), [line 15](/Arduino%20Sketch/Musical_wristband_rev0.2.ino#L15), as long as it is not the same pin as the sensor pin.

Similary, the NeoPixel DIN pin can be redefined as any other I/O pin in the [final sketch](/Arduino%20Sketch/Musical_wristband_rev0.2.ino), [line 17](/Arduino%20Sketch/Musical_wristband_rev0.2.ino#L17), as long as it is not the same pin as the sensor pin.
...
<ul>3.8. Connect the Arduino Pro Micro to PC via micro USB cable, open the Arduino IDE and upload the sketch to the board. Make sure you choose the right Port and Board under the Tools menu of the IDE.</ul>
